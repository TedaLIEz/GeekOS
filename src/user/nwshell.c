...
static void chld_handler(int n) {
}
static void usr1_handler(int n) {
    int pid, ret;
...
int main() {
    ...
    Signal(chld_handler, SIGCHLD);
    Signal(usr1_handler, SIGUSR1);
...
void Spawn_Single_Command(struct Process *proc, const char *path) {
    ...
    Print("nwshell spawning command: %s\n", proc->command);
    proc->pid =
    ...
    if(pid < 0) {
        ...
        Print
            ("Spawn_With_Path returned 0 which is an unlikely pid that I cannot wait for.\n");
    } else {
        if(background) {
            Print("[%d]\n", pid);
        } else {
            Print("nwshell spawed %s and waiting for pid %d\n",
                  proc->program, pid);
