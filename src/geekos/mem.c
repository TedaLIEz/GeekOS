...
static void *Alloc_Or_Reclaim_Page(pte_t * entry, ulong_t vaddr,
  ...
  start_over:
    ...
    if(paddr != 0) {
        ...
        if(Is_Mmaped_Page(page->context, page->vaddr)) {
            ...
            if(pagefileIndex < 0) {
                ...
                /* KASSERT0(paddr == NULL, "expected to return NULL when no space on the paging file."); */
                paddr = NULL;
                page->flags |= PAGE_PAGEABLE;
                Unlock_Page(page);
                goto done;
            ...
            Debug
                ("Writing physical frame %p to free page on paging file at %d from %p at 0x%lx\n",
                 paddr, pagefileIndex, page->context, page->vaddr);
            Enable_Interrupts();        /* writing expects interrupts enabled; were disabled above for this fn. */
            KASSERT0(!(page->flags & PAGE_PAGEABLE),
                     "can't write out a pageable page; must be not pageable for now.");
            KASSERT0(page == Get_Page((ulong_t) paddr),
                     "another assertion.");
        ...
        if(page->flags & PAGE_ALLOCATED) {
            ...
            /* free list should be super empty, so this check may be efficient enough 
               to run without the mutex. */
            /* 
               if(Is_Member_Of_Page_List(&s_freeList, page)) {
               KASSERT0(false, "ERR: Page being reclaimed is on the free list.");
               }
             */
        }
    ...
    if(pinnedPage) {
        // Debug("locking page at %p as pinned, %s\n", paddr, (page->flags & PAGE_ALLOCATED) ? "allocated": "unallocated");
        //   page->flags |= PAGE_LOCKED;  
        // Print("e\n");
        page->flags &= ~(PAGE_PAGEABLE);        /*ns14; not using LOCKED allows the page to be Free()d. */
...
void *Alloc_Page(void) {
    ...
    KASSERT0(Interrupts_Enabled(), "expect interrupts enabled before alloc_or_reclaim_page");   /* because of APF */
